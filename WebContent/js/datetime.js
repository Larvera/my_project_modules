...
/**
 * 날짜를 마스크에 따라 포맷
 * 마스크에서 9는 any number character에 대응
 * ex) formatDate("20020527","9999-99-99");
 */
...
/*
    ...
    return toTimeString(date);
*/
    return shiftTime(getTime(),y,m,d,h);
}

/**
 * 두 Time이 몇개월 차이나는지 구함
 ...
 * 현재 시각을 마스크 형식으로 리턴
 */
function getTime(mask) {
    return formatDate(getSystemTime(),mask);
}
...
function getDate(mask) {
    return formatDate(getSystemDate(),mask);
}
